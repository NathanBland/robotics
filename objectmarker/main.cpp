...
int main(int argc, char** argv){
    ...
    input_directory = argv[2];//"/home/joel/Sphero/positive.txt"; //
    output_file = "positive.txt"; //argv[1];"/home/joel/Sphero/SpheroPositive/";//
    ...
    ofstream output;
    output.open(output_file.c_str(), ios::app);
    fprintf(stderr, "done.\n");
    ...
    while((dir_entry_p = readdir(dir_p)) != NULL){
        ...
        if((image=cvLoadImage(strPrefix.c_str(),1)) != 0){
            fprintf(stderr,"cv loaded image");
...

            {
                ...
                if(numOfRec>0 && iKey=='b'){
                    if(output.is_open()){
                        ...
                        fprintf(stderr, "%s %d%s\n", strPrefix.c_str(), numOfRec, strPostfix.c_str());
                        cvReleaseImage(&image);
                    }else {
